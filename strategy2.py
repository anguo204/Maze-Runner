...
k = 0
random_q_vals =  1 #makes another matrix of (nxn) of random q values
#print(random_q_vals)
def testfn(val,x,y,k): #function to count the number of neighbors that are on fire
    ...
    if(y < n-1 and val[x][y+1] == 2):
        k += 1
    formula = 1 - (1-random_q_vals)**k #use the formula with q values
...
def reset(arr, value):
    for n in path:
        x = n[0]
        y = n[1]
        arr[x][y].setFill("white")

count = 0
with open('0.6vsolbe.csv', 'wb') as csvfile:
    Cx=0
    for i in range(0,15): #making the two timelines of the mazes
        count = count +1
        newMaze = np.zeros([n,n])
        newMaze2 = np.zeros([n,n])
        w.getMouse()
        reset(arr,path)
        for i in range(0,n):
            for j in range(0,n):
                ...
                if(T == True and val[i][j] != 0):
                    ...
                    newMaze[i][j] = val[i][j]
                if newMaze[i][j] == 2:
                    newMaze2[i][j] == 0 #creating the new "walls where the fire is
                else:
                    newMaze2[i][j] = newMaze[i][j]
        coloring(arr,newMaze,n) #calling the coloring function
        val = newMaze
        path, a = strategy2(newMaze2,arr,n)
        print(path)
        if( a== None):
            filewriter = csv.writer(csvfile, delimiter=',',quotechar='|', quoting=csv.QUOTE_MINIMAL)
            filewriter.writerow([str(random_q_vals), str(count)])
            count = 0
